    ...
    public static void main(String[] args) throws Exception {
        ...
        do{
            ...
            switch(opcao){
                ...
                case 1:
                    System.out.println("\n>>>Opção escolhida foi 1. Cadastrar um livro.<<<");//Messagem de opçãa escolhida
                    cadastrarLivro(tituloLivro, autorLivro, anoPublicacao); //Chama o metodo que cadastra um livro
                    break;
                //Caso opção 2 seja digitada
                case 2:
                    System.out.println("\n>>>Opção escolhida foi 2. Pesquisar Livro.<<<");//Messagem de opçãa escolhida
                    ...
                    if (indice == -1) { //Se o valor retornado pelo metodo for -1 mostra a menssagem de livro não cadastrado
                        System.out.println("Esse livro não está cadastrado!");
                    } else { //Se o valor retornado pelo metodo for o Indice no array do livro mostra o livro cadastrado
                ...
                case 3:
                    System.out.println("\n>>>Opção escolhida foi 3. Remover um Livro.<<<");//Mesagem de opçãa escolhida
                    ...
                    removerLivro(tituloLivro, autorLivro, anoPublicacao, tituloRemover);//Chama o metodo de remoção do livro
                    break;
                //Caso opção 4 seja digitada
                case 4:
                    System.out.println("\n>>>Opção escolhida foi 4. Listar Livros.<<<");//Mesagem de opçãa escolhida
                ...
                default:
                    System.out.println(">>>Digite uma opção valida!!!<<<");//Mesagem casa o usuario não digite nunhma opçaõ disponivel
                    break;
            }
        }while(opcao !=5);//A condição é caso a opção digitada seja diferente de 5 o loop/Programa continue rodando caso contrario ele encerra
    }
    ...
    public static void listarLivros(String[] tituloLivro, String[] autorLivro, int[] anoPublicacao){
        ...
        for (int i = 0; i < tituloLivro.length; i++) { //percorre o array
            if (tituloLivro[i] != null) { //verifica se o elemento não é nulo
                ...
                System.out.println(contador + ". " + tituloLivro[i] + " - " + autorLivro[i] + " (" + anoPublicacao[i] + ")"); //mostra as informações do livro
            }
        }
